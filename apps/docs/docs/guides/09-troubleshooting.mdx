...
```text
The registry at http://localhost:3001/r/metrics.json did not answer.
```

**Cause.** Deckard has no public registry host. The URL in `components.json` points at the docs site, which has to be running.

**Fix.** Start it, in a checkout of the Deckard repository:

```bash
pnpm --filter docs dev
```

Or point `deckard add block` somewhere else with `--registry <url>`. The URL carries a `{name}` placeholder.
