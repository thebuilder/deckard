...
<TypeTable
  type={{
    ...
    motion: {
      type: '"auto" | "frozen"',
      default: '"auto"',
      description: "The default every slide takes. \"frozen\" holds every motion background in the deck on one fixed frame. Reduced motion and the capture commands freeze one either way, and the presenter preview mounts none.",
    },
    routes: {
