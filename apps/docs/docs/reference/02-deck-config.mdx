---
title: Deck config
description: Every field defineDeck accepts, what it resolves them into, and the markup the deck header and footer render.
sidebar:
  icon: settings
---
...
`"counter"` named the footer back when a visible footer also carried previous and next buttons. Those moved into the deck controls, so `"counter"` and `"visible"` now describe the same footer, and a deck that still says `"counter"` resolves to `"visible"`.
...
| `header` `footer` `layout` `background` | The slide's own value, or the deck default. |
