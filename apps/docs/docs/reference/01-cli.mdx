...
- **The chrome band.** Nothing inside `[data-slide-frame]` may cross the top of `[data-slide-footer]` or the bottom of `[data-slide-header]`. Absolutely positioned parts are skipped, and so is a slide whose `layout` is `"fullscreen"`, where full-bleed content under the chrome is the point.
...
The measurement is `measureSlideLayout` from `@deckard/core`, which is the same function [the amber ring](/guides/the-canvas) runs in development, so this gate and `next dev` cannot disagree.
