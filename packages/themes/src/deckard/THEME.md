...
soft shadow. In light mode the shadow does most of the separating, since the
hairline border alone is not enough on a white sheet.
...
`accent` background. It is the only accent colour in the theme.
...
canvas pixels. Deckard has no source template of its own, so it sits on the
contract defaults in `@deckard/core/styles.css` and restates them:
`--slide-title-size` `6rem`, `--slide-heading-size` `4.25rem`,
...
`--slide-figure-size` is `7.5rem` and `--slide-figure-unit-size` is `3.5rem`,
both the contract values.

`--slide-chrome-size` is `1.5rem`. The contract sizes chrome off
`--slide-label-size`, and this theme's labels are `1.5rem` too, so the explicit
value restates it.
...
`--slide-label-tracking` is the letter spacing on uppercase labels. At `0.3em` it
is wider than the contract's `0.25em`. Lowering it below `0.2em` makes eyebrows
read as ordinary small text.
...
pale sheet, so this file paints no radial wash, veil, or corner glow, and no
`--slide-wash`, `--slide-veil`, `--slide-glow`, or `--slide-spotlight` token
stands behind one.
...
`--muted-foreground` with `-0.03em` of tracking: larger than a heading and
quieter than one. No other block part is styled here, so the blocks look the way
the blocks set them. If a `ProseSlide` rail or a `LogList` status
looks wrong here, fix the block rather than this file.
...
progress bar moves to the bottom edge of the canvas and fills in `--primary`.
