...
| --- | --- | --- |
| `background` | `string` | The card. |
| `foreground` | `string` | The slide title. |
| `muted` | `string` | The brand, the slide position, and the description line. |
| `accent` | `string` | The rule above the title. |
...
The colors take hex or `rgb()`. The renderer does not parse `oklch()` or `var()`, and `defineDeck` fails on either, naming the theme and the field. Pick the palette the deck opens in: the card is one image, so it cannot follow the reader's color mode.

Each built-in's card takes `--background`, `--foreground`, `--muted-foreground`, and `--primary` from the palette of the mode it opens in, converted to hex, and the first family in `--slide-font-heading`.
