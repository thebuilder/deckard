...
Slide blocks are the other piece, and deliberately not a package. They install into your app as source through a shadcn registry, and you edit them from then on.

:::warning
None of the packages are published to npm yet. The [Quickstart](/quickstart) has the tarball path that works today.
:::
