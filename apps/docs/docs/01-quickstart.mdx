...
:::warning
`@thebuilder/deckard-core`, `@thebuilder/deckard-themes`, and `@thebuilder/deckard-cli` are built for npm but not published there yet, so the `npx` line below does not work from a clean machine. [Install from tarballs](#installing-before-the-packages-publish) until they land.
:::
...
| `--core-tarball <path>` | `^<cli version>` | Install `@thebuilder/deckard-core` from a local tarball. |
...
```bash
pnpm cli:build
pnpm --filter @thebuilder/deckard-core exec pnpm pack --pack-destination /tmp/deckard
pnpm --filter @thebuilder/deckard-themes exec pnpm pack --pack-destination /tmp/deckard
pnpm --filter @thebuilder/deckard-cli exec pnpm pack --pack-destination /tmp/deckard

node packages/cli/bin/deckard.mjs init ~/code/my-talk \
  --core-tarball /tmp/deckard/thebuilder-deckard-core-0.0.1.tgz \
  --themes-tarball /tmp/deckard/thebuilder-deckard-themes-0.0.1.tgz \
  --cli-tarball /tmp/deckard/thebuilder-deckard-cli-0.0.1.tgz
```

The generated app is identical either way. When the packages publish, `npx @thebuilder/deckard-cli init` becomes the whole install and nothing in the app changes.
...
<Steps>
  <Step title="Install the packages">
    ...
    pnpm add -D @thebuilder/deckard-cli
    ```
  </Step>
