...
- A flow window: the previous two slides, the current one, and the next five, by title. Click any of them to jump the deck there.
...
:::note
Hidden steps stay in the layout, so a stepped slide takes its full height from the first step. That is deliberate, since content that reflows as it reveals is worse than content that fades in, but it does mean [`check-overflow`](/reference/cli#deckard-check-overflow) measures the fully revealed slide.
:::
