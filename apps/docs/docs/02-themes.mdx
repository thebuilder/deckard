...
Print, not screen. Warm newsprint in light mode, warm ink in dark. Everything is serif and everything is flat: corners are all but square and there is no shadow at all, so a content card is a panel with a rule around it. The border sits two steps darker than the default, because a flat card on flat paper needs the line to do all the work.
