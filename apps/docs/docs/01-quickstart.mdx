...
- my-talk/
  - app/
    - slides/
      - blocks/ (the seven block families, yours to edit)
      - [id]/page.tsx
    - presenter/page.tsx
    - page.tsx
    - sitemap.ts
...
The blocks come in either way. `init` ships them inside the CLI package and copies them into `app/slides/blocks/`. Adding a block *later* does need a registry, which today means the Deckard docs site running locally on port 3001. See [Blocks and the registry](/guides/blocks-and-the-registry).
