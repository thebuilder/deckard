...
Every size is set against the 1920x1080 canvas at a 16px root, so `1rem` is 16 canvas pixels. A 6rem title is 96 canvas pixels, about a fifth of the canvas height.
...
A theme has four textures to work with: flat colour, a rule, a grid, and a hatch. A radial gradient reads as a smudge on a projector and as grey on paper.
