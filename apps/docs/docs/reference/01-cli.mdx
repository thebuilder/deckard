...
Reads `out/screenshots/manifest.json` and composes `out/contact-sheet.png`. It builds and serves nothing, so run `deckard screenshots` first.
...
:::note
`contact-sheet` also accepts `--light`, but never reads it: the color mode comes from the manifest. It does not accept `--port` or `--skip-build`, despite what the built-in help text implies about "the four checks above".
:::
...
Note the inversion: the checks default to dark, the PDF defaults to light, because a handout gets printed. Writes `out/slides.pdf`, or wherever `PDF_EXPORT_OUTPUT` points, one slide to a page and sized from the canvas.
...
A name outside the six comes back with the six:
...
Deckard has no public registry host yet, so the URL `init` writes points at the docs site on port 3001. When the registry does not answer, the command tells you to start that site.
