...
Each frame is a slide canvas painted by that theme's own stylesheet, in the color mode the theme opens on. Follow one into the [theme gallery](/themes) for the same six at a size you can read, with a light and dark switch on every card and the palette behind it.
...
A deck does not need this. Reach for it on a page that renders a slide outside the runtime, such as the [theme gallery](/themes) on this site, which builds a scaled-down canvas and lets the six stylesheets paint it.
...
The four textures a theme has are flat colour, a rule, a grid, and a diagonal hatch.
