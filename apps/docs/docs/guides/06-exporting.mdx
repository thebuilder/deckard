---
title: Exporting
description: PDF handouts, per-slide screenshots, and the contact sheet. All three run the deck in a real browser at canvas scale.
sidebar:
  icon: image-down
---
...
One page per slide at the deck's canvas size, captured from the canvas element. `1920x1080` becomes `1440x810pt`, which is the same thing at 96dpi. The page size comes from the `canvas` config in `deck/deck.ts`, so the export cannot drift from what the audience saw.
...
| `--port <n>` | `3410`, or `PDF_EXPORT_PORT` | |

The light default is deliberate: a handout gets printed.
...
:::note
The export uses its own build profile, separate from the one the screenshot and overflow commands share, because it needs the export environment variables compiled in. Expect a build the first time even if you just ran `deckard screenshots`.
:::
...
This is the check that catches what no linter can. Three slides in a row that all end above the halfway line, or the same numbered list twice back to back, are invisible one slide at a time and obvious in a grid.

## Build reuse

The screenshot, overflow, and PDF commands share one harness. It stamps `.next/deckard-build.json` after a build and compares that timestamp against the newest file under `app/`, `assets/`, `components/`, `deck/`, `hooks/`, `lib/`, and `public/`, plus `components.json`, `next.config.*`, `package.json`, `postcss.config.*`, `tsconfig.json`, and the resolved `@deckard/core` and its `src/`. A stale stamp triggers `next build`.
