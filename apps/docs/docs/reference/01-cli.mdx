...
Creates `deck/theme/` with `theme.css` and `THEME.md` copied out of the installed `@thebuilder/deckard-themes`, generates an `index.ts` exporting a `SlideTheme` literal, the built-in's share card `card` values included, and rewrites `deck/deck.ts` to `import { theme } from "@/deck/theme"`.
