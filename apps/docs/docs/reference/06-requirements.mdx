...
A deck is a Next.js app with `@deckard/core` as a dependency. Everything below is what that app needs.
...
| Next.js | 16 or newer | `peerDependencies` on `@deckard/core` |
| React | 19 or newer | `peerDependencies` on `@deckard/core` |
| React DOM | 19 or newer | `peerDependencies` on `@deckard/core` |
| Tailwind CSS | 4 | `@deckard/core/styles.css` uses `@source`, which is v4 only |
...
Playwright itself is a dependency of `@deckard/cli` and installs with it. The browser binary is a separate download:

```bash
pnpm exec playwright install chromium
```

Without it those four commands fail with `Failed to launch Chromium` and the install line for your package manager. Nothing else in the CLI needs a browser: `init`, `validate`, `doctor`, `add`, and `eject` do not.
...
What is true: the route adapters in `@deckard/core/next` import no Node APIs, and the slide route reads nothing from the request. `@deckard/core/code-block` is the part to check first if you try it, because shiki loads a WebAssembly regex engine there.
