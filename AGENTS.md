...
- `packages/themes` is `@thebuilder/deckard-themes`, the built-in deck themes. Each one is a
  `theme.css`, an `index.ts` exporting one `SlideTheme`, and a `THEME.md`, plus a
  generated `card-colors.ts` holding the share card colors in hex. After a
  palette change run `pnpm --filter @thebuilder/deckard-themes generate:cards`;
  the package test fails while a generated file is stale. The palette parser in
  `scripts/theme-palette.ts` is also what the docs theme gallery reads, by
  relative path. That reach across packages is for build tooling only, so the
  file keeps no relative imports of its own; runtime code still goes through
...
- Apps import the runtime through the package exports only: `@thebuilder/deckard-core`, `/components`, `/code-block`, `/discovery`, `/layout`, `/next`, `/share-card`, `/slide-from-module`, `/ui`, `/utils`, `/styles.css`, and `@thebuilder/deckard-themes` for the presets. Never a deep path into `packages/core/src` or `packages/themes/src`.
...
- Route files belong to `@thebuilder/deckard-core/next`. An app's `app/slides/[id]/page.tsx`, `app/presenter/page.tsx`, `app/sitemap.ts`, and `app/page.tsx` are re-exports of `createSlideRoute`, `createPresenterPage`, `createDeckSitemap`, and `createFirstSlideRedirect`. `app/slides/[id]/opengraph-image.tsx` re-exports `createSlideShareCard` from `@thebuilder/deckard-core/share-card`, an entry of its own because `next/og` loads a WebAssembly renderer. The share card route is optional, so `deckard doctor` does not require it. Every absolute URL a deck publishes, the sitemap and the `metadataBase` in the root layout and the slide route, comes from `deckSiteUrl()` in `@thebuilder/deckard-core`. `apps/playground` is the one exception: its two decks, `/slides` and `/example`, render the slide page themselves so the theme picker can hand the canvas another built-in, and still take `generateMetadata` and `generateStaticParams` from `createSlideRoute`. The switch is in `apps/playground/components/theme-switch/`, and `deckard init` writes the re-export. A slide URL takes `?theme=<id>` and `?mode=light|dark`, and a page that frames the deck hides the in-deck picker and can post `{ type: "deckard:view", theme, mode }` to it, which is how the docs site drives its embedded decks.
