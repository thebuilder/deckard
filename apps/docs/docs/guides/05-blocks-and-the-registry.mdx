...
description: Slide blocks install into your app as source through a shadcn registry. What the eight items contain, how to install them, and where they are served from today.
sidebar:
  icon: blocks
---
...
Expect to edit them. `HeroSlide` caps its headline with `text-balance` tuned for a sans display face; on a 4.5rem serif you will want to change that, and you can, because it is your file.
...
`preset-blocks` is the one to run. Alongside the seven block files it adds `@import "@deckard/core/styles.css"` to your app stylesheet, which carries the token contract and registers the package's own compiled output as a Tailwind source. That import is the whole build wiring: no `transpilePackages` entry to add, no `@source` line.

Every block is left aligned and fills the padded frame. `HeroCenteredSlide` and `MinimalBreakerSlide` are the two that centre. A measure is capped in one place, `ProseSlide`, where running copy needs it, and the cap is in canvas pixels rather than `ch`.

Themes are not registry items. They ship as `@deckard/themes`, and a deck gets one by importing it.
