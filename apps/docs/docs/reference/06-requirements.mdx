...
The share cards export too, as a PNG file named `opengraph-image` with no extension inside each slide's folder. Configure the host to serve those files as `image/png`, since a host that picks the type by extension will not.
