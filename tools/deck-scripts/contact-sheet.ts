...
import { fail, readNumberFlag, write } from "./lib/cli.ts"
import { projectRoot } from "./lib/paths.ts"
import { launchBrowser } from "./lib/preview.ts"
import {
  readManifest,
...
const outputPath = path.resolve(projectRoot, "out/contact-sheet.png")
const pagePath = path.join(screenshotDirectory, "contact-sheet.html")
const columns = readNumberFlag(process.argv, "columns", 4)
...
function composeHtml(manifest: ScreenshotManifest) {
...
async function composeContactSheet() {
  ...
  fs.writeFileSync(pagePath, composeHtml(manifest))
...
composeContactSheet().catch(fail)
