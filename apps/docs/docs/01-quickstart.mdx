...
The blocks are not affected by any of this. `init` ships them inside the CLI package and copies them in, because a brand-new app has no shadcn config yet. Adding a block later fetches from the registry the docs site serves. See [Blocks and the registry](/guides/blocks-and-the-registry).
