...
The copied `index.ts` carries the built-in's [share card](#share-cards) as literal values. Editing the palette in `theme.css` leaves the card as it was, so change `card` in the same edit.
...
| `card` | `SlideThemeCard`, optional | The colors and face of each slide's [share card](#share-cards). Without it the card is grayscale in the default face. |
...
A deck with an `app/slides/[id]/opengraph-image.tsx` route renders one Open Graph image per slide at build time, from [`createSlideShareCard`](/reference/core#createslidesharecard). The card renderer reads no stylesheet, so a theme repeats what the card needs in `card`:
...
export const theme = {
  card: {
    ...
    font: { family: "Schibsted Grotesk", weight: 700 },
  },
  className: "my-theme",
  // ...
} satisfies SlideTheme
...
| --- | --- | --- |
| `colors.light`, `colors.dark` | the four colors below, per mode | The card paints the theme's home mode: its `defaultColorMode`, or `light` for `"system"`. |
| `colors.<mode>.background` | `string` | The card. |
| `colors.<mode>.foreground` | `string` | The slide title. |
| `colors.<mode>.muted` | `string` | The brand, the slide position, and the description line. |
| `colors.<mode>.accent` | `string` | The rule above the title. |
| `font.family` | `string` | The face the card text is set in. A family Google Fonts carries, spelled the way it lists it. |
| `font.weight` | `100` to `900`, in hundreds | The title's weight. The rest of the card is set at 400. |
| `font.uppercase` | `boolean`, optional | Capitalises the title, for a theme whose headings are set in capitals. |

The colors take hex or `rgb()`. The renderer does not parse `oklch()` or `var()`, and `defineDeck` fails on either, naming the theme, the mode, and the field. The card is one image, so it cannot follow the reader's color mode: it paints the mode `homeColorMode(theme)` names, and changing `defaultColorMode` changes the card with it.

Each built-in's card takes `--background`, `--foreground`, `--muted-foreground`, and `--primary` from both palettes, converted to hex, and the first family in `--slide-font-heading`.
...
Four more values name the moment a slide is rather than how it looks. Mark the opener `background: "hero"`, each section break `"breaker"`, the one sentence the deck is built around `"statement"`, and the last slide `"closing"`. A theme that paints the role in `motion`, such as aurora, shows its field there. Every other theme renders the role as a variant from the table above:

| Role | Renders as without a field |
| --- | --- |
| `"hero"` | `"default"` |
| `"breaker"` | `"spotlight"` |
| `"statement"` | `"default"` |
...
The canvas and the background layer carry the resolved name, so a theme that paints no field for `"closing"` styles it through its `"accent"` rules. A theme that adds a field keys `motion` by the role and styles `[data-slide-background="<role>"]` under it. The [slide metadata reference](/reference/slide-metadata#background-roles) has the full contract.
